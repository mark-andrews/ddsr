fa_parallel <- function(x,n.obs=NULL,fm="minres",fa="both",nfactors=1,main="Parallel Analysis Scree Plots",n.iter=20,error.bars=FALSE,se.bars=FALSE,SMC=FALSE,ylabel=NULL,show.legend=TRUE,sim=TRUE,quant=.95,cor="cor",use="pairwise",plot=TRUE,correct=.5)  { 
  ...
  # exchange mclapply for lapply
  templist <- lapply(1:n.iter,function(XX) { #at least for now, the errors from mixedCor prevent mclapply 
