...
To use this properly, we must include the expression with the `{}` after the command.
In other words, to obtain $\sqrt{2}$, we must use `$\sqrt{2}$`.
Had we written `$\sqrt 2$` or even `$\sqrt(2)$`, we would have obtained $\sqrt(2)$, which is not the desired result.
...
knitr::include_graphics(file.path(chapter_wd, "include_graphics/github_screenshot.png"))
```
